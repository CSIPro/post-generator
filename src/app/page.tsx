...
const HomeScreenLink: FC<LinkProps> = ({ href, children }) => {
  return (
    <Link
      href={href}
      className="block w-fit rounded-sm bg-white px-4 py-2 text-center text-lg font-bold uppercase tracking-wider text-primary transition-all hover:bg-accent hover:text-xl hover:text-white"
    >
      {children}
    </Link>
  );
